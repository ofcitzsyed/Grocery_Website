...
import { FaPhoneAlt, FaHome, FaFire, FaPercentage, FaPaperPlane } from 'react-icons/fa';
import { MdArrowDropDown } from 'react-icons/md';
...
const Header = () => {
  ...
  useEffect(() => {
    ...
    if (token && userId) {
      ...
      fetch(`http://localhost:5000/api/cart?user_id=${userId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
        ...
        .then((data) => {
          if (Array.isArray(data)) {
            ...
            console.error("Cart not in array format", data);
          }
        })
        .catch((err) => console.error("Error fetching cart data:", err));
    }
  ...
  return (
    <div className="w-full shadow-sm bg-white sticky top-0 z-50">
      ...
      <div className="flex items-center justify-between px-12 py-2">
        ...
        {/* Search */}
        <div className="flex w-[450px] bg-gray-100 rounded overflow-hidden">
          <div className="px-3 flex items-center gap-1 text-sm font-medium text-gray-700 border-r">
            All Categories
            <ChevronDown className="w-4 h-4" />
          </div>
          <input
        ...
        {/* Cart and Profile */}
        <div className="flex items-center space-x-6">
          <div className="flex items-center space-x-1 text-sm text-gray-800 cursor-pointer" onClick={() => navigate('/cart')}>
            <div className="relative">
              <ShoppingCart className="w-5 h-5" />
          ...
          {user ? (
            ...
            <p className="text-sm text-gray-800 cursor-pointer" onClick={() => navigate('/login')}>
              Login
            </p>
          )}
        </div>
      ...
      <nav className="w-full flex items-center justify-between px-12 py-3 bg-white">
        ...
        <ul className="flex items-center gap-8 text-sm font-medium">
          <li className="flex items-center gap-1 text-green-500">
            <FaHome size={14} />
            <span className="text-sm text-gray-800 cursor-pointer" onClick={() => navigate('/home')}>Home</span>
          </li>
          <li className="flex items-center gap-1 text-gray-700 hover:text-green-500 transition">
            <FaFire size={14} />
            <span>Hot deals</span>
